<template>
  <view class="container">
    ...
    <view class="input-section">
      <view class="section-title">粘贴快手分享链接</view>
      <textarea
        v-model="shareUrl"
        class="url-input"
        placeholder="请粘贴快手分享链接，如：https://v.kuaishou.com/xxx/"
        :maxlength="500"
      />
    ...
    <view class="tips-section">
      ...
      <view class="tips-list">
        <view class="tip-item">1. 打开快手APP，找到要下载的视频</view>
...
import { parseKuaishouUrl, getKuaishouDownloadUrl } from '@/api/kuaishou';
...
const proxyVideoUrl = computed(() => {
  ...
  return getKuaishouDownloadUrl(videoInfo.value.videoUrl);
});
...
const parseUrl = async () => {
  ...
  try {
    const res = await parseKuaishouUrl(shareUrl.value.trim());
...
const downloadVideo = async () => {
  ...
  try {
    const tempFilePath = await request.download(
      getKuaishouDownloadUrl(videoInfo.value.videoUrl),
      (progress) => {
...
const saveAllImages = async () => {
  ...
  for (let i = 0; i < videoInfo.value.images.length; i++) {
    ...
    try {
      // 使用后端代理下载图片，解决防盗链问题
      const tempFilePath = await request.download(
        getKuaishouDownloadUrl(videoInfo.value.images[i])
      );
