  ...
  <view class="container">
    ...
    <view class="cookie-section">
      ...
      <view class="cookie-warning" v-if="!cookieStatus.isLogin">
        ...
        <view class="warning-text">
          <text class="warning-title">当前为低画质模式（360P）</text>
          <text class="warning-desc">设置Cookie后可获取高清画质（1080P）</text>
        </view>
      </view>

      <!-- 已登录信息 -->
      <view class="cookie-info" v-if="cookieStatus.isLogin">
        <text class="info-item" v-if="cookieStatus.userId">{{ cookieStatus.userId }}</text>
        <text class="info-item" v-if="cookieStatus.level">Lv{{ cookieStatus.level }}</text>
        <text class="info-item vip" v-if="cookieStatus.isVip">大会员</text>
        <text class="info-item quality">高清画质可用</text>
      </view>
    ...
    <view class="modal-mask" v-if="showGuide" @click="showGuide = false">
      <view class="modal-content guide-modal" @click.stop>
        <view class="modal-title">获取Cookie指引</view>
        <view class="guide-steps">
          <view class="guide-step">
            <view class="step-num">1</view>
            <view class="step-content">
              <text class="step-title">打开B站网页</text>
              <text class="step-desc">浏览器访问 bilibili.com 并登录账号</text>
            </view>
          </view>
          <view class="guide-step">
          ...
          <view class="guide-step">
            ...
            <view class="step-content">
              <text class="step-title">切换到Network标签</text>
              <text class="step-desc">点击 Network（网络）标签页</text>
            </view>
          </view>
          <view class="guide-step">
            <view class="step-num">4</view>
            <view class="step-content">
              <text class="step-title">刷新页面</text>
              <text class="step-desc">按 F5 刷新页面，等待请求加载</text>
            </view>
          </view>
          <view class="guide-step">
            <view class="step-num">5</view>
            <view class="step-content">
              <text class="step-title">找到任意请求</text>
              <text class="step-desc">点击任意请求，查看 Request Headers</text>
            </view>
          </view>
          <view class="guide-step">
            ...
            <view class="step-content">
              ...
              <text class="step-desc">找到 Cookie 字段，复制完整内容</text>
            </view>
          </view>
        </view>
        <view class="guide-tip">
          <text>💡 Cookie必须包含 SESSDATA 才能正常使用</text>
        </view>
    ...
    <view class="modal-mask" v-if="showCookieInput" @click="showCookieInput = false">
      <view class="modal-content" @click.stop>
        <view class="modal-title">设置B站Cookie</view>
        ...
        <textarea
          v-model="cookieInput"
          class="cookie-input"
          placeholder="请粘贴B站Cookie，必须包含SESSDATA"
          :maxlength="5000"
        />
        <view class="modal-tips">
          <text>提示：Cookie存储在本地，有效期约30天</text>
        </view>
        <view class="modal-btns">
    ...
    <view class="input-section">
      <view class="section-title">粘贴B站分享链接</view>
      <textarea
        v-model="shareUrl"
        class="url-input"
        placeholder="请粘贴B站分享链接，如：https://b23.tv/xxx"
        :maxlength="500"
      />
    ...
    <view class="preview-section" v-if="videoInfo">
      <view class="section-header">
        ...
        <view class="copy-btn" @click="copyText">复制标题</view>
      </view>
      <view class="video-card">
        <image class="video-cover" :src="proxyCoverUrl" mode="aspectFill" />
        <view class="video-info">
          <view class="video-title">{{ videoInfo.title }}</view>
          <view class="video-author">@{{ videoInfo.author }}</view>
          <view class="video-duration" v-if="videoInfo.duration">时长: {{ formatDuration(videoInfo.duration) }}</view>
        </view>
      </view>
    ...
    <view class="tips-section">
      ...
      <view class="tips-list">
        <view class="tip-item">1. 未设置Cookie时默认低画质（360P）</view>
        <view class="tip-item">2. 设置Cookie后可获取高清画质（1080P）</view>
        <view class="tip-item">3. 大会员账号可获取4K画质</view>
        <view class="tip-item">4. 打开B站，复制视频分享链接</view>
        <view class="tip-item">5. 粘贴链接并解析，预览后保存</view>
        <view class="tip-item tip-notice">视频归B站及作者所有，本应用不存储任何视频</view>
      </view>
    </view>
  </view>
</template>
...
import { parseBilibiliUrl, getBilibiliDownloadUrl, getBilibiliVideoUrl, getBilibiliCoverUrl, checkBilibiliCookie, saveCookieLocal, getCookieLocal, clearCookieLocal } from '@/api/bilibili';
...
onMounted(async () => {
  ...
  if (localCookie.value) {
    ...
    setTimeout(() => {
      uni.showModal({
        title: '提示',
        content: '当前为低画质模式（360P），设置Cookie后可获取高清画质（1080P）。是否查看获取Cookie指引？',
        confirmText: '查看指引',
        cancelText: '稍后再说',
        success: (modalRes) => {
          if (modalRes.confirm) {
...
const proxyVideoUrl = computed(() => {
  ...
  return getBilibiliVideoUrl(videoInfo.value.videoUrl);
});

// 代理封面URL（解决小程序域名白名单问题）
const proxyCoverUrl = computed(() => {
  if (!videoInfo.value?.cover) return '';
  return getBilibiliCoverUrl(videoInfo.value.cover);
});
...
const checkCookieStatus = async () => {
  ...
  try {
    const res = await checkBilibiliCookie(localCookie.value);
...
const saveCookie = async () => {
  ...
  // 检查是否包含SESSDATA
  if (!cookieInput.value.includes('SESSDATA')) {
    uni.showToast({ title: 'Cookie缺少SESSDATA', icon: 'none' });
    return;
  }
  ...
  try {
    // 先验证Cookie
    const res = await checkBilibiliCookie(cookieInput.value.trim());
    ...
    if (res.data.isLogin) {
      ...
      cookieInput.value = '';
      uni.showToast({ title: '登录成功，高清可用', icon: 'success' });
    } else {
...
const clearCookie = () => {
  uni.showModal({
    title: '确认清除',
    content: '清除后将恢复为低画质模式，确定清除Cookie吗？',
    success: (res) => {
      if (res.confirm) {
        clearCookieLocal();
        localCookie.value = '';
...
const parseUrl = async () => {
  ...
  try {
    // 传递本地Cookie给后端
    const res = await parseBilibiliUrl(shareUrl.value.trim(), localCookie.value);
...
const downloadVideo = async () => {
  ...
  try {
    const tempFilePath = await request.download(
      getBilibiliDownloadUrl(videoInfo.value.videoUrl),
      (progress) => {
